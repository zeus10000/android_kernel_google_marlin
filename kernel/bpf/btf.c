...
/* 4.4 conflict resolution: bpf_verifier_log infrastructure not available.
 * Map bpf_verifier_vlog -> btf_vlog, using our own log state.
 */
static void btf_vlog(struct btf_verifier_env *env, const char *fmt,
		     va_list args)
{
	struct btf_log *log = &env->log;
	u32 avail;
	char *kbuf;

	if (!log->level || !log->ubuf || log->len_written >= log->len_total - 1)
		return;
	avail = log->len_total - log->len_written - 1;
	kbuf = kmalloc(avail, GFP_KERNEL | __GFP_NOWARN);
	if (!kbuf)
		return;
	vscnprintf(kbuf, avail, fmt, args);
	if (copy_to_user(log->ubuf + log->len_written, kbuf, strlen(kbuf) + 1))
		{} /* best-effort */
	log->len_written += strlen(kbuf);
	kfree(kbuf);
}
...
__printf(2, 3) static void __btf_verifier_log(struct btf_verifier_env *env,
					      const char *fmt, ...)
{
	va_list args;

	va_start(args, fmt);
	btf_vlog(env, fmt, args);
	va_end(args);
}
						   ...
						   const char *fmt, ...)
{
	...
	__btf_verifier_log(env, "[%u] %s %s%s",
			   env->log_type_id,
			   btf_kind_str[kind],
				    ...
				    const char *fmt, ...)
{
	...
	__btf_verifier_log(env, "\t%s type_id=%u bits_offset=%u",
...
static void btf_verifier_log_hdr(struct btf_verifier_env *env)
{
	...
	hdr = btf->hdr;
	__btf_verifier_log(env, "magic: 0x%x\n", hdr->magic);
	__btf_verifier_log(env, "version: %u\n", hdr->version);
	__btf_verifier_log(env, "flags: 0x%x\n", hdr->flags);
	__btf_verifier_log(env, "parent_label: %u\n", hdr->parent_label);
	__btf_verifier_log(env, "parent_name: %u\n", hdr->parent_name);
	__btf_verifier_log(env, "label_off: %u\n", hdr->label_off);
	__btf_verifier_log(env, "object_off: %u\n", hdr->object_off);
	__btf_verifier_log(env, "func_off: %u\n", hdr->func_off);
	__btf_verifier_log(env, "type_off: %u\n", hdr->type_off);
	__btf_verifier_log(env, "str_off: %u\n", hdr->str_off);
	__btf_verifier_log(env, "str_len: %u\n", hdr->str_len);
	__btf_verifier_log(env, "btf_total_size: %u\n", btf->data_size);
}
			     ...
			     u32 log_level, char __user *log_ubuf, u32 log_size)
{
	...
	if (log_level || log_ubuf || log_size) {
		...
		if (env->log.len_total < 128 ||
		    env->log.len_total > UINT_MAX >> 8 ||
		    !env->log.level || !env->log.ubuf) {
			err = -EINVAL;
			goto errout;
		}
	}
	...
	if (!err && env->log.level &&
	    env->log.len_written >= env->log.len_total - 1) {
		err = -ENOSPC;
		goto errout;
	}
